    ...
    <div className="flex flex-col items-start gap-4 md:gap-[21px] w-full md:w-[626px] md:mx-auto md:mt-5">
      ...
      <div className="flex items-center gap-2 md:gap-2 flex-wrap w-full">
        {problem.tags.map((tag, tagIndex) => (
          ...
          >
            <img className="w-3 h-3 " alt="Danger" src="/images/danger.svg" />
...
export const ProblemSolutionsOverviewSection = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // End of scroll (top to bottom): last section visible but its top above center → last content
      const lastSection = sections[sections.length - 1];
      if (lastSection) {
        const lastRect = lastSection.getBoundingClientRect();
        if (lastRect.top < window.innerHeight / 2 && lastRect.bottom > 0) {
  ...
  return (
    <section className="containerSection gap-0">
      ...
      <div className="flex flex-col gap-8 w-full">
        ...
        <div className="relative flex w-full flex-row lg:flex-row lg:items-start lg:gap-10 lg:pt-[80px]">
          {/* Left: scrollable problem summaries (desktop: scroll-text for active detection) */}
          <div className="flex w-full flex-col gap-8 md:gap-20 pb-6 md:pb-10 lg:flex-1 lg:max-w-[50%] lg:pt-[10vh] lg:pb-[40vh]">
            {problemsData.map((problem, index) => (
              <div
                key={index}
                data-index={index}
                className="scroll-text flex flex-col gap-8 md:gap-20 pb-6 md:pb-10 w-full lg:min-h-[50vh] lg:justify-start"
              >
                {/* <Separator className="h-0.5 bg-white/10" /> */}
                ...
                <div className="flex flex-col lg:flex-row items-start gap-6 md:gap-10 py-0.5 w-full">
                  ...
                  {/* Right card on mobile: one per row (no sticky) */}
                  <div className="relative w-full lg:hidden lg:w-[726.13px] lg:h-[512.11px] overflow-hidden shrink-0">
                    <div className="w-full flex items-center border border-solid border-card-border1">
                      <Card className="w-full rounded-xl overflow-hidden border border-solid border-card-border bg-[linear-gradient(180deg,rgba(132,135,142,0.08)_1%,rgba(94,94,94,0.08)_100%)]">
          ...
          <div className="hidden lg:flex sticky top-[150px] h-[90vh] w-full max-w-[726.13px] shrink-0 justify-center items-start pt-0.5">
            {problemsData.map((problem, index) => (
              ...
              >
                <div className="relative w-full max-w-[726.13px] h-[512.11px] overflow-hidden">
                  <div className="absolute top-0.5 left-0.5 w-full lg:w-[723px] h-[509px] flex items-center border border-solid border-card-border1">
